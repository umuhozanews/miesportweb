...
export default async function WCOverviewPage({ params }: Props) {
  ...
  if (year !== "2026") return <Empty label={`No data available for ${year}`} />;
  ...
  const featuredMatches = fixtures.length > 0 ? fixtures.slice(0, 4) : results.slice(0, 4);
    ...
    <div style={{ padding: "1.25rem" }}>
      {featuredMatches.length > 0 && (
        <section style={{ marginBottom: "1.5rem" }}>
          <SectionLabel>{fixtures.length > 0 ? "Upcoming Matches" : "Recent Results"}</SectionLabel>
...
function FeaturedMatch({ event: e }: { event: EspnEvent }) {
  ...
  return (
    <div style={{ background: "#1a1a1a", border: "1px solid #222", borderRadius: 10, padding: "14px 16px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <TeamCard logo={e.homeTeam.logo} name={e.homeTeam.name} align="left" bold={isFt && hs > as_} dim={isFt && hs < as_} />
        ...
        <div style={{ textAlign: "center", padding: "0 10px", flexShrink: 0 }}>
          {isFt || isLive ? (
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 800, color: "#60a5fa" }}>{espnFmtTime(e.startTimestamp)}</div>
              <div style={{ fontSize: 10, color: "#3a3a3a", marginTop: 2 }}>{espnFmtDate(e.startTimestamp)}</div>
            </div>
          )}
        </div>

        <TeamCard logo={e.awayTeam.logo} name={e.awayTeam.name} align="right" bold={isFt && as_ > hs} dim={isFt && as_ < hs} />
      </div>
    </div>
  );
}
...
function GroupStandingsPreview({ group, year }: { group: EspnGroup; year: string }) {
