...
import {
  getESPNWCFixturesByYear, getESPNWCStandingsByYear, getWCDateRange,
...
export default async function WCOverviewPage({ params }: Props) {
  ...
  // ESPN path for all supported years (2014, 2018, 2022, 2026)
  if (getWCDateRange(year)) {
    ...
    if (featuredMatches.length === 0 && !firstGroup) {
      return <Empty label={year === "2026" ? "Tournament begins June 11, 2026" : `No data available for ${year}`} />;
    }
